...
from net.net import AutoEncoderModel, AutoEncoderModelExtended
...
class Trainer:
    ...
    def __init__(self, exp_name, epochs, batch_size, dataroot, lr, num_classes, isresume) -> None:
        ...
        #--------- get data loaders
        dset = MNISTPair(root=dataroot,sumflag=True,is_train=True)
        length = len(dset)
        tl = int(0.8*length)
        ...
        self.clf = AutoEncoderModelExtended(input_dim=1, encoded_dim=128, out_channel=num_classes).to( self.device)
    ...
    def __training_loop(self, epoch, io):
        ...
        for i , (data1, data2, sumtensor, label) in enumerate(self.train_loader):
            ...
            x1, x2, pred = self.clf(data1, data2, sumtensor)

            classfication_loss = F.cross_entropy(pred, label, weight = self.cls_weights)
            recon_loss = (F.smooth_l1_loss(x1, data1) + F.smooth_l1_loss(x2, data2)) * 0.5
            loss = classfication_loss + recon_loss
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()
            class_loss += classfication_loss.detach().item()
            latent_loss += recon_loss.detach().item()
            ...
            if i%100 == 0:
                log = 'Training epoch:{} iteration: {} class_loss:{} latent_loss:{}'.format(epoch, i, classfication_loss, recon_loss)
                io.cprint(log)

        time_str = 'Train end:' + ctime(time())
        io.cprint(time_str)

        class_loss /= len(self.train_loader)
        latent_loss /= len(self.train_loader)
    ...
    def __validation_loop(self, epoch, io):
        ...
        for i , (data1, data2, sumtensor, label) in enumerate(self.val_loader):
            ...
            if i%100 == 0:
                log = 'Validation epoch:{} iteration: {} class_loss:{} latent_loss:{}'.format(epoch, i, classfication_loss, recon_loss)
                io.cprint(log)
                fake.extend(x1.detach().cpu().numpy())
                org.extend(data1.detach().cpu().numpy())
        
        time_str = 'Validation end:' + ctime(time())
        io.cprint(time_str)

        class_loss /= len(self.val_loader)
        latent_loss /= len(self.val_loader)
    ...
    def train(self, io):
        ...
        for epoch in range(self.epochs):
            ...
            self.writer.add_scalar('train/class_loss', pack['class_loss'], epoch)
            self.writer.add_scalar('train/latent_loss', pack['latent_loss'], epoch)
            self.writer.add_scalar('train/precision', pack['prec'], epoch)
            self.writer.add_scalar('train/recall', pack['recall'], epoch)
            log = 'Training epoch:{} class_loss:{} latent_loss:{}'.format(epoch, pack['class_loss'], pack['latent_loss'])
            log += '\n %s'%pack['report']
            io.cprint(log)

            pack = self.__validation_loop(epoch, io)
            self.writer.add_scalar('valid/class_loss', pack['class_loss'], epoch)
            self.writer.add_scalar('valid/latent_loss', pack['latent_loss'], epoch)
            self.writer.add_scalar('valid/precision', pack['prec'], epoch)
            self.writer.add_scalar('valid/recall', pack['recall'], epoch)
            log = 'Validation epoch:{} class_loss:{} latent_loss:{}'.format(epoch, pack['class_loss'], pack['latent_loss'])
            log += '\n %s'%pack['report']
            io.cprint(log)
...
if __name__ == '__main__':
    ...
    trainer = Trainer(args.exp_name, args.epochs, args.batch_size, args.dataroot, args.lr, args.num_classes, args.resume)
